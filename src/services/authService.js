...
exports.changePassword = expressAsyncHandler(async (req, res, next) => {
   const password = await bcrypt.hash(req.body.password, 10)
   const response = await User.findOneAndUpdate({ email: 'user@gmail.com' }, {
      password,
...
exports.protect = expressAsyncHandler(async (req, res, next) => {
   ...
   if (changePasswordAt > decoded.iat) {
      return next(new ApiError('you are change password yet, please login again', 403))
   }
...
exports.allowTo = (...args) => expressAsyncHandler((req, res, next) => {
   if (!args.includes(req.user.role)) {
      return next(new ApiError(`you have no permission for this route ${req.originalUrl}`, 403))
   }
   next()
})
