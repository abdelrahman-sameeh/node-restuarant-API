...
exports.createAddressValidator = [
   // user => userId
   check('user')
      .notEmpty().withMessage('Address must be belong to a user')
   ...
   check('alias')
      ...
      .isLength({ min: 2 }).withMessage('alias length must be more than 2 characters'),
   check('details')
      .notEmpty().withMessage('alias is required'),
   check('phone')
      .notEmpty().withMessage('phone is required')
      .isMobilePhone(['ar-EG']).withMessage('accept only EGY phone number'),
   check('city')
